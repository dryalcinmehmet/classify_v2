...
from .visualize_cm import plot_opt
...
def simulated_annealing(
    ...
    deterministic: bool = False) -> OptimizationResult:
    """
    ...
    ----------
    current_cm : np.ndarray
    current_perm : list, None
    save_score_plot: bool, save score for plot
    ...
    if temp <= 0.0:
        raise ValueError(
            logger.error("Temp: %d needs to be positive" % (temp) 
            )
        )
    
    if cooling_factor <= 0.0 or cooling_factor >= 1.0:
        raise ValueError(
            logger.error("Cooling factor: %d needs to be in the interval "
                         "(0, 1)" % (cooling_factor)
            )
        )
    
    n = len(current_cm)
    logger.info("Count of label: %i" % n)
    ...
    s = []
    cs = []
    logger.info("## Starting Score: %.2f" % current_score)
    for step in range(steps):
        ...
        if chance <= hot_prob_thresh:
            ...
            if changed:
                cs.append(current_score)
                s.append(step)
    
    # save plot optimization scores 
    if save_score_plot:
        plot_opt(s, cs)
    
    logger.info("## Finishing Score: %.2f" % current_score)
...
def move_1d(
    ...
    if not (insert_pos < from_start or insert_pos > from_end):
        raise ValueError(
            logger.error("insert_pos=%d needs to be smaller than from_start=%d"
            " or greater than from_end=%d" % (insert_pos, from_start, from_end)
            )
        )
